...
def test_hardcoded_names(verbose=True, **kwargs):
    ...
    assert py2tex('numpy.arccos(x)', print_latex=False) == '$$\\arccos\\left(x\\right)$$'
    # the test below uses unicode symbol, which is valid only in Python2
    if sys.version_info[0] != 3:
        assert py2tex('arcsin(α)', print_latex=False) == '$$\\arcsin\\left(\\alpha\\right)$$'
        assert py2tex('arctan(α)', print_latex=False) == '$$\\arctan\\left(\\alpha\\right)$$'
    else:
        assert py2tex('arcsin(alpha)', print_latex=False) == '$$\\arcsin\\left(\\alpha\\right)$$'
        assert py2tex('arctan(alpha)', print_latex=False) == '$$\\arctan\\left(\\alpha\\right)$$'
